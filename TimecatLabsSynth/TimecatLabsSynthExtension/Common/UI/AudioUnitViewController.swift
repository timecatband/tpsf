...
private let log = Logger(subsystem: "timecatlabs.TimecatLabsSynthExtension", category: "AudioUnitViewController")

public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<TimecatLabsSynthExtensionMainView>?
	...
	*/
    ...
    public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
        audioUnit = try TimecatLabsSynthExtensionAudioUnit(componentDescription: componentDescription, options: [])
        
        guard let audioUnit = self.audioUnit as? TimecatLabsSynthExtensionAudioUnit else {
            log.error("Unable to create TimecatLabsSynthExtensionAudioUnit")
            return audioUnit!
        }
        ...
        audioUnit.setupParameterTree(TimecatLabsSynthExtensionParameterSpecs.createAUParameterTree())
    ...
    private func configureSwiftUIView(audioUnit: AUAudioUnit) {
        ...
        let fileChosenCallback: (URL) -> Void = { [weak self] url in
            guard let audioUnit = self?.audioUnit as? TimecatLabsSynthExtensionAudioUnit else {
                return
            }
            audioUnit.setModel(url.path)
        }
        let effectFileChosenCallback: (URL) -> Void = { [weak self] url in
            guard let audioUnit = self?.audioUnit as? TimecatLabsSynthExtensionAudioUnit else {
                return
            }
            audioUnit.setEffectModel(url.path)
        }
        let content = TimecatLabsSynthExtensionMainView(parameterTree: observableParameterTree, fileChosenCallback: fileChosenCallback, effectFileChosenCallback: effectFileChosenCallback)        
